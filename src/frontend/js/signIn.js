...
const authorizationInit = () => {
    ...
    function loginAutorize() {
        ...
        xhr.onload = () => {
            ...
            if (xhr.readyState === 4) {
                //статус который вам будет в ответ приходить, от него отталкиваnpmемся что нам делать, вытаскиваем данные через xhr.body
                if (xhr.status === 200) {
                    document.location.href = 'http://localhost:7070/index.html';
